...
describe(`🧱 ${FontAwesomeLayersText.name}`, () => {
	...
	describe('correctly handles properties', () => {
		...
		describe('`position` property', () => {
			...
			const [TOP_RIGHT_CLASS, TOP_LEFT_CLASS, BOTTOM_RIGHT_CLASS, BOTTOM_LEFT_CLASS] = [
				'top-right',
				'top-left',
				'bottom-right',
				'bottom-left'
			...
			test(NOT_PROVIDED_CASE, () => {
				...
				expect(
					[...text.classList].every(
						(item) =>
							![TOP_RIGHT_CLASS, TOP_LEFT_CLASS, BOTTOM_RIGHT_CLASS, BOTTOM_LEFT_CLASS].includes(
								item
							)
					)
				).toBe(true);
